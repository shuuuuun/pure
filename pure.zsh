...
prompt_pure_preexec() {
	...
	[[ $PURE_DISABLE_SET_TITLE ]] && $PURE_DISABLE_SET_TITLE || prompt_pure_set_title 'ignore-escape' "$PWD:t: $2"
}
...
prompt_pure_preprompt_render() {
	...
	# Set the path.
	preprompt_parts+=('$PURE_PREPROMPT_LEFT %F{${PURE_PREPROMPT_DIRECTORY_COLOR:-blue}}%~%f')
...
prompt_pure_setup() {
	...
	PROMPT+="$PURE_PROMPT_LEFT%(?.%F{${PURE_PROMPT_SYMBOL_COLOR:-magenta}}.%F{${PURE_PROMPT_SYMBOL_ERROR_COLOR:-red}})${PURE_PROMPT_SYMBOL:-❯}%f "
}
